...
function computeLayout(rect: DOMRect): Layout {
  ...
  const BW = Math.floor(rectW.width);
  const BH = Math.floor(rectW.height);
  ...
  // ③ 缩放系数，保证三张+两缝+两侧 safe 和 高度方向都放得下
  ...
  // ⑥ 水平位置：整体居中 + 两侧 safe，内部两段 gap
  const totalWidth = 3 * cardW + 2 * gap;
  const usable = BW;
  const startX = Math.max(safe, Math.floor((usable - totalWidth) / 2));
  const lefts = [
    startX,
    startX + cardW + gap,
    startX + (cardW + gap) * 2,
  ...
  // ⑦ 垂直居中 + safe
  const top = Math.max(safe, Math.floor((BH - cardH) / 2));
